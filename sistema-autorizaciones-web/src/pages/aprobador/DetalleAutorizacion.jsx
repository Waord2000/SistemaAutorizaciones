...
export default function DetalleAprobador() {
  const { id }  = useParams()
  ...
  const manejarCancelar = async () => {
    ...
    try {
      ...
      setExito('Autorización cancelada.')
  ...
  return (
    <div>
      ...
      <div style={estilos.estadoBanner}>
        ...
        <div style={estilos.estadoItem}>
          <span style={estilos.estadoLabel}>Mi firma:</span>
      ...
      <div style={estilos.seccion}>
        ...
        <div style={estilos.grid3}>
          <div style={estilos.campoLectura}>
            ...
            <span style={estilos.campoValor}>{autorizacion.fecha}</span>
          </div>
          <div style={estilos.campoLectura}>
      ...
      {/* ── Sección 2 ── */}
      <div style={estilos.seccion}>
        <h2 style={estilos.seccionTitulo}>2. Descripción de la Tarea</h2>
        <div style={estilos.campoLectura}>
        ...
        <div style={{ ...estilos.campoLectura, marginTop: '12px' }}>
          <span style={estilos.campoLabel}>Descripción detallada</span>
      ...
      {/* ── Sección 3 ── */}
      <div style={estilos.seccion}>
        <h2 style={estilos.seccionTitulo}>3. Personal Involucrado</h2>
        <div style={estilos.grid3}>
          <div style={estilos.campoLectura}>
            <span style={estilos.campoLabel}>Técnico responsable</span>
            <span style={estilos.campoValor}>
      ...
      <div style={estilos.seccion}>
        ...
        <div style={estilos.tagsContenedor}>
          {autorizacion.epps?.map(epp => (
            <span key={epp} style={estilos.tagEpp}>🦺 {epp}</span>
          ))}
        </div>
      ...
      {/* ── Sección 5 ── */}
      <div style={estilos.seccion}>
        <h2 style={estilos.seccionTitulo}>5. Análisis de Riesgos</h2>
          ...
          <tbody>
            {autorizacion.analisisRiesgos?.map((r, i) => (
              <tr key={r.id} style={estilos.tablaFila}>
                ...
                <td style={estilos.tablaTd}>{r.riesgoIdentificado}</td>
                ...
                <td style={estilos.tablaTd}>
                  <span style={{
                    ...
                    color: nivelRiesgoColor[r.nivelRiesgo]?.color,
                  }}>
                    {r.nivelRiesgo}
                  </span>
                </td>
      ...
      {autorizacion.historial?.length > 0 && (
        <div style={estilos.seccion}>
          <h2 style={estilos.seccionTitulo}>Historial de cambios</h2>
          {autorizacion.historial.map(h => (
            <div key={h.id} style={estilos.historialItem}>
              <div style={estilos.historialFecha}>
                {new Date(h.fechaAccion).toLocaleString('es-GT')}
              </div>
              <div style={estilos.historialDetalle}>
                <strong>{h.usuario}</strong> — {h.comentario}
                {h.estadoAnterior && (
                  <span style={estilos.historialEstados}>
...
const estilos = {
  ...
  historialItem:       { display: 'flex', gap: '16px', padding: '12px 0', borderBottom: '1px solid #F0F4F8' },
  historialFecha:      { fontSize: '12px', color: '#9CA3AF', whiteSpace: 'nowrap', minWidth: '160px' },
  historialDetalle:    { fontSize: '14px', color: '#374151' },
  historialEstados:    { marginLeft: '8px', backgroundColor: '#EBF8FF', color: '#2B6CB0', borderRadius: '12px', padding: '2px 8px', fontSize: '12px', fontWeight: '600' },
  acciones:            { display: 'flex', gap: '12px', marginTop: '24px' },
  btnCompletar:        { backgroundColor: '#059669', color: '#fff', border: 'none', borderRadius: '8px', padding: '12px 24px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' },
  btnCancelar:         { backgroundColor: '#FEE2E2', color: '#991B1B', border: 'none', borderRadius: '8px', padding: '12px 24px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' },
